...
function CanteenDashboard() {
  ...
  const fetchMenu = async () => {
    ...
    try {
      ...
      // keep original sorting behaviour
  ...
  return (
    ...
    >
      ...
      {canteens[0] && (
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
          <div>
            ...
            >
              {isCanteenOpenNow(canteens[0]) ? "Currently Open" : "Currently Closed"}
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-white text-sm sm:text-base ${
                canteens[0].isOpen ? "bg-red-600 hover:bg-red-700" : "bg-green-600 hover:bg-green-700"
              }`}
            >
      ...
      <div className="flex gap-3 mb-6">
        <button
          className={`px-4 py-2 rounded-lg text-sm sm:text-base ${activeTab === "menu" ? "bg-blue-600 text-white" : "bg-gray-200 text-black"}`}
          onClick={() => setActiveTab("menu")}
        >
          Menu
        </button>
        <button
          className={`px-4 py-2 rounded-lg text-sm sm:text-base ${activeTab === "orders" ? "bg-blue-600 text-white" : "bg-gray-200 text-black"}`}
      ...
      {activeTab === "menu" && (
        <>
          <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-md mb-8">
            <h2 className="text-lg sm:text-xl font-semibold mb-4">Add Menu Item</h2>
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <input
                placeholder="Name"
                value={newItem.name}
                onChange={(e) => setNewItem({ ...newItem, name: e.target.value })}
                className="border p-2 sm:p-3 rounded-xl flex-1 text-sm sm:text-base"
              />
              <input
                placeholder="Price"
                value={newItem.price}
              ...
              <button
                ...
                className="bg-green-600 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-xl"
              >
                Add
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {menu.map((m) => (
              <div
                key={m._id}
                className={`group relative flex flex-col items-center text-center rounded-2xl shadow-lg overflow-hidden transition-all duration-300 hover:scale-[1.02] ${m.isAvailable ? "bg-white/95" : "bg-gray-100/90 opacity-80 grayscale"}`}
              >
                {/* Image */}
                {m.photo ? (
                  <img
                    src={m.photo}
                    alt={m.name}
                    className="w-full h-48 sm:h-64 object-cover rounded-t-2xl transition-transform duration-300 group-hover:scale-105"
                  />
                ) : (
                  <div className="w-full h-48 sm:h-64 bg-gray-300 flex items-center justify-center text-gray-600 rounded-t-2xl">
                    No Image
                  </div>
                )}
                ...
                <div className="p-4 sm:p-5 w-full flex flex-col items-center">
                  ...
                  <div className="flex items-center justify-center gap-3 mt-3 flex-wrap">
                    <div className="flex items-center border border-gray-300 rounded-lg px-2 py-1 focus-within:ring-2 focus-within:ring-blue-400 transition-all">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setMenu((prev) => prev.map((it) => (it._id === m._id ? { ...it, price: val } : it)));
                          // then call API to persist
                          handleUpdatePrice(m, val);
                        }}
                    ...
                    <button
                      ...
                      className={`px-4 py-2 font-semibold rounded-lg shadow-sm transition-colors duration-200 ${m.isAvailable ? "bg-green-500 hover:bg-green-600 text-white" : "bg-red-500 hover:bg-red-600 text-white"}`}
                    >
        ...
        <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-md">
          {orders.length === 0 ? (
            ...
            orders.map((order) => (
              <div
                key={order._id}
                className="border p-4 sm:p-5 rounded-xl flex flex-col gap-3 mb-4 shadow-sm bg-gray-50 hover:bg-gray-100 transition"
              >
                {/* Top info */}
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                  <div>
                    ...
                    <p className="text-sm text-gray-700">Customer: {order.user?.name || "Unknown"}</p>
                  </div>
                  ...
                  <div className="flex flex-col items-end">
                    ...
                    <span className="text-gray-700 font-semibold mt-1">₹{order.amount}</span>
                  </div>
                </div>
